 ...
 * ===========================================================
 * Documentation Statement: No help received. Used the slides and write-up.
 * =========================================================== */
...
void* SJFcpu(void* param) {
    ...
    while (1) {
        ...
        if (p == NULL) {
            ...
            // Find the shortest job and run it
...
void* NPPcpu(void* param) {
    ...
    while (1) {
        ...
        // ── Selection (only when idle) ───────────────────────────────────
        // NPP is non-preemptive (by name): once a process is running (p != NULL) we
        // never replace it mid-burst.  We only enter this block when the CPU
        // has nothing to run.
        if (p == NULL) {
            // Lock readyQ before inspecting or modifying it — another CPU
            ...
            // Find the highest priority job and run it
...
void* RRcpu(void* param) {
    ...
    Process* p = NULL;
    int default_quantum = svars->quantum;
    int quantum = 0;
    ...
    while (1) {
        ...
        if (quantum == 0 && p != NULL) {
            pthread_mutex_lock(&(svars->readyQLock));
            // Check queue
            int queueIsEmpty = svars->readyQ.head == NULL;

            if (queueIsEmpty) { // Queue is empty
                // Let the process run for another time quantum
                quantum = default_quantum;
                printf("Scheduling PID %d\n", p->PID);
            } else {
                // Put the process back into the readyQ
                p->requeued = true;
